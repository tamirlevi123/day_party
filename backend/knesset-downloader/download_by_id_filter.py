"""
Filter-based download: Use $filter=DocumentBillID gt <max_id> instead of $skip
This avoids pagination issues with sparse IDs.

Strategy:
1. Order by DocumentBillID asc
2. Filter: DocumentBillID gt <max_id_seen_so_far>
3. Take top 100
...
def main():
    ...
    print(f"\nStrategy: Use $filter=DocumentBillID gt <max_id> instead of $skip")
    ...
    # Start with max_id we have, or 0 if we have nothing
    current_max_id = max_document_bill_id
    
    print(f"\nStarting filter-based download (starting from DocumentBillID > {current_max_id:,})...")
    
    while True:
        call_number += 1
        
        # Build filter: DocumentBillID gt current_max_id
        ...
        # This ensures next batch starts from where we left off
